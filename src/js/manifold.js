/* ===========================================================================
   ...
   The manifold from the Rust crate (raum) = WavePlaneScene:
     ...
     • the projective ray — NEW: a line through the origin, oriented by a
                            cursor-driven quaternion (slerp'd each frame),
                            extended past the frustum so it runs edge-to-edge.
   ...
   Drop-in via bg.js (the router), which falls back to washi.js when WebGPU is
   unavailable. Caps DPR at 2, respects prefers-reduced-motion (freezes auto
   animation; cursor-orbit + ray stay live).
   =========================================================================== */

const WGSL = /* wgsl */ `
struct U {
  viewProj: mat4x4<f32>,
  ray:  vec4<f32>,       // orientation quaternion (x,y,z,w), slerp'd on the CPU
...
const murasaki = vec3<f32>(0.416, 0.298, 0.769);  // #6a4cc4 — the lilac accent
...
const RAY_LEN:     f32 = 80.0;  // half-length of the projective ray (runs past the frustum)
const RAY_ALPHA:   f32 = 0.85;
...
// =========================================================================
//  projective ray — line through the origin, oriented by u.ray (quaternion)
// =========================================================================
struct ROut { @builtin(position) pos: vec4<f32>, @location(0) tparam: f32 };

@vertex
fn vs_ray(@location(0) axis: vec3<f32>) -> ROut {
  let p = qrot(u.ray, axis * RAY_LEN);     // axis is (±1,0,0); rotate the base direction
  var o: ROut;
  o.pos = u.viewProj * vec4<f32>(p, 1.0);
  o.tparam = axis.x;                       // -1 … +1 along the ray
  return o;
}
@fragment
fn fs_ray(in: ROut) -> @location(0) vec4<f32> {
  let glow = 1.0 - abs(in.tparam);         // brightest at the origin
  let col = mix(murasaki, gofun, glow * 0.4);
  return vec4<f32>(col, RAY_ALPHA);
}
`;
...
// ---- quaternion helpers (the ray "moves via quaternion") ----------------
function qAxisAngle(ax, ang) { const h = ang / 2, s = Math.sin(h); return [ax[0] * s, ax[1] * s, ax[2] * s, Math.cos(h)]; }
function qMul(a, b) {
  return [
...
function qSlerp(a, b, t) {
  ...
  const th0 = Math.acos(d), th = th0 * t, st0 = Math.sin(th0), st = Math.sin(th);
  const s0 = Math.cos(th) - d * st / st0, s1 = st / st0;
...
const RAY_VERTS = new Float32Array([-1, 0, 0,  1, 0, 0]);   // base axis, scaled+rotated in the shader
function buildWells() {
  const n = 10, v = [], idx = [];
  for (let i = 0; i < n; i++) {
    const x = i - n / 2 + 0.5;
...
export async function init() {
  ...
  const rayVB   = mk(RAY_VERTS,   GPUBufferUsage.VERTEX);

  const uniBuf = device.createBuffer({ size: 96, usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST });
  const uni = new Float32Array(24);   // 0..15 viewProj · 16..19 ray quat · 20 time
  ...
  // ray drawn last, depth-test off so it spears cleanly through everything
  const rayPipe = device.createRenderPipeline({
    layout: pipeLayout,
    vertex: { module, entryPoint: 'vs_ray', buffers: [{ arrayStride: 12, attributes: [{ shaderLocation: 0, offset: 0, format: 'float32x3' }] }] },
    fragment: { module, entryPoint: 'fs_ray', targets: [{ format, blend: alphaOver }] },
    primitive: { topology: 'line-list' },
    depthStencil: depthState(false, 'always'),
  });
  ...
  let curQ = [0, 0, 0, 1];   // ray orientation, slerp'd toward the cursor target
  ...
  function frame(now) {
    ...
    // ray quaternion: build a target from the (raw) cursor, slerp toward it
    const yaw   = (targetM.x - 0.5) * Math.PI * 2.0;
    const pitch = (targetM.y - 0.5) * Math.PI;
    const targetQ = qMul(qAxisAngle([0, 1, 0], yaw), qAxisAngle([0, 0, 1], pitch));
    curQ = qSlerp(curQ, targetQ, 0.06);

    uni.set(viewProj, 0);
    uni[16] = curQ[0]; uni[17] = curQ[1]; uni[18] = curQ[2]; uni[19] = curQ[3];
    uni[20] = t;
    ...
    const pass = encoder.beginRenderPass({
      colorAttachments: [{ view: ctx.getCurrentTexture().createView(), loadOp: 'clear', storeOp: 'store',
        clearValue: { r: 0.937, g: 0.925, b: 0.890, a: 1 } }],
